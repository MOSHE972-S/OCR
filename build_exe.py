"""
הרץ קובץ זה לאחר שהפרויקט עובד:
    python build_exe.py

יצור: dist/MyCustomOCR/MyCustomOCR.exe
"""
...
def build():
    with open("MyCustomOCR.spec", "w", encoding="utf-8") as f:
        f.write(SPEC)
    print("✅ קובץ spec נוצר")
    print("🔨 בונה EXE... (5-10 דקות)")
    result = subprocess.run(
        [sys.executable, "-m", "PyInstaller", "--noconfirm", "--clean", "MyCustomOCR.spec"]
    )
    if result.returncode == 0:
        print("\n✅ EXE נוצר: dist\\MyCustomOCR\\MyCustomOCR.exe")
        print("📁 שתף את כל תיקיית dist\\MyCustomOCR\\")
    else:
        print("\n❌ הבנייה נכשלה. בדוק את הפלט מעלה.")
